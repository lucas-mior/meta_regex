...
regoff_t
re_search(struct re_pattern_buffer *bufp, char *string, Idx length, Idx start,
...
regoff_t
re_match_2(struct re_pattern_buffer *bufp, char *string1, Idx length1,
           char *string2, Idx length2, Idx start, struct re_registers *regs,
           Idx stop) {
...
regoff_t
re_search_2(struct re_pattern_buffer *bufp, char *string1, Idx length1,
...
build_sifted_states(re_match_context_t *mctx, re_sift_context_t *sctx,
    ...
    for (i = 0; i < cur_src->nelem; i++) {
        ...
        if (!naccepted
            ...
            && ((sctx->sifted_states[str_idx + 1]) != NULL
                && re_node_set_contains(
                    &(sctx->sifted_states[str_idx + 1])->nodes,
                    dfa->nexts[prev_node]))) {
            naccepted = 1;
        }
    ...
    for (node_idx = 0; node_idx < candidates->nelem; ++node_idx) {
        ...
        do {
            ...
            if (to_idx > sctx->last_str_idx
                ...
                || !((sctx->sifted_states[to_idx]) != NULL
                     && re_node_set_contains(
                         &(sctx->sifted_states[to_idx])->nodes, dst_node))
