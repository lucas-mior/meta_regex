...
meta_regex_match(MetaRegex *regex, uint8 *input, int32 input_len,
    ...
    case MATCH_ALGO_BTNFA: {
        if (regex->has_start_anchor) {
            result
                = match_btnfa(regex, input, input_len, 0, pmatch_len, pmatch);
            if (result == 0) {
                return 0;
            }
            return REG_NOMATCH;
    ...
    case MATCH_ALGO_LAZY_DFA: {
        ...
        for (int32 j = 0;; j += 1) {
            ...
            if (bit_match || regex->can_be_null) {
                result = match_lazy_dfa(regex, input, input_len, j, pmatch_len,
                                        pmatch);
                if (result == 0) {
                    return 0;
                }
    ...
    case MATCH_ALGO_STATIC_DFA: {
        if (regex->has_start_anchor) {
            result = match_static_dfa(regex, input, input_len, 0, pmatch_len,
                                      pmatch);
            if (result == 0) {
                return 0;
            }
