...
match_at_recursive(MetaOp *ops, char *original_string, char *current_string, size_t nmatch, regmatch_t pmatch[]) {
    ...
    if (ops[0].type == META_OP_ALTERNATION) {
        MetaOp *end_op = ops; int32 depth = 0;
    ...
    if (ops[0].type == META_OP_GROUP_START) {
        int32 group_id = ops[0].value; int32 old_so = -1; int32 res;
    ...
    if (ops[0].type == META_OP_GROUP_END) {
        int32 group_id = ops[0].value; int32 old_eo = -1; int32 res;
        if (pmatch != NULL && (size_t)group_id < nmatch) {
    ...
    }
    {
        ...
        if (is_star || is_plus || is_opt || is_bound) {
            MetaOp token = ops[0]; MetaOp *next_ops = ops + 2; char *s = current_string; char *max_s;
            regmatch_t pmatch_backup[32]; int32 min_req = 0; int32 max_req = -1; int32 consumed = 0;
            ...
            int32 count = 0;
            while (count < min_req) {
                if (!matches_char(token, s, &consumed)) return -1;
                s += consumed; count += 1;
            }
            max_s = s;
            while ((max_req == -1 || count < max_req) && matches_char(token, max_s, &consumed)) {
                max_s += consumed; count += 1;
            }
            if (pmatch != NULL) {
                size_t copy_size = (nmatch > 32) ? 32 : nmatch;
                for (size_t i = 0; i < copy_size; i += 1) pmatch_backup[i] = pmatch[i];
            ...
            while (max_s >= s) {
                ...
                if (max_s == s) break;
                char *prev = max_s - 1;
                while (prev > s && (*(unsigned char *)prev & 0xC0) == 0x80) prev -= 1;
                max_s = prev;
            }
            return -1;
        }
    ...
    int32 consumed = 0;
    if (matches_char(ops[0], current_string, &consumed)) return match_at_recursive(ops + 1, original_string, current_string + consumed, nmatch, pmatch);
    return -1;
}
