...
bench_dup_cstr(char *s) {
    ...
    char *copy = malloc2((usize)len + 1);
    memcpy(copy, s, (usize)len + 1);
    return copy;
}
...
bench_load_generated_inputs(char *array_name, char *path,
    ...
    storage = malloc2((usize)file_size + 1);
    read_size = fread(storage, 1, (usize)file_size, file);
    ...
    free2(storage, (usize)file_size + 1);
    return;
}
    ...
    for (int32 i = 0; i < loaded->count; i += 1) {
        if (loaded->cases[i].name != NULL) {
            free2(loaded->cases[i].name,
                  (usize)strlen32(loaded->cases[i].name) + 1);
        }
        if (loaded->cases[i].input != NULL) {
            free2(loaded->cases[i].input,
                  (usize)strlen32(loaded->cases[i].input) + 1);
        }
    }
